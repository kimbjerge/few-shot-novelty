...
if __name__=='__main__':
    ...
    parser.add_argument('--seed', default=0, type=int) # Radom seed number used to generate random tasks of samples for learning and test
    ...
    for modelName in os.listdir(args.modelDir):
        if ('.pth' in modelName) and (alphaStr in modelName):
            ...
            resFileName = args.model + '_' + args.dataset + '_' + str(int(args.alpha*10)) + "_novelty_rand_learn.txt"
            line = "ModelDir,ModelName,FewShotClassifier,Way,Shot,Query,Accuracy,BayesThreshold,Average,Std,AverageOutlier,StdOutlier,MeanBetween,Seed\n"
            ...
            line += str(threshold) + ',' + str(avg) + ',' + str(std) + ',' + str(avg_o) + ',' + str(std_o) + ',' + str(abs(avg-avg_o)) + ',' + str(args.seed) + '\n'
            print(line)
            resFile.write(line)        
            resFile.close()
            ...
            resFileName =  args.model + '_' +  args.dataset + '_' + str(int(args.alpha*10)) + "_novelty_rand_test.txt"
            line = "ModelDir,Model,FewShotClassifier,Novelty,Way,Shot,Query,Accuracy,Precision,Recall,F1,TP,FP,FN,Method,Threshold,Alpha,Seed,ModelName\n"
            if os.path.exists(resDir+subDir+resFileName):
                resFile = open(resDir+subDir+resFileName, "a")
            else:
                resFile = open(resDir+subDir+resFileName, "w")
            ...
            #for n_shot in [5]: # Test with 5 and 1 shot               
                for use_novelty in [True, False]: # Test with and without novelty
                    ...
                    for few_shot in few_shot_classifiers:
                        ...
                        line += args.threshold + ',' + str(threshold) + ',' + str(args.alpha) + ',' + str(args.seed) + ',' + args.modelDir + '/' + modelName +  '\n'
                        print(line)
                        resFile.write(line)    
                        resFile.flush()
