...
if __name__=='__main__':
    ...
    for modelName in os.listdir(args.modelDir):
        if '.pth' in modelName:
            ...
            resFileName = args.model + '_' + args.dataset + "_novelty_bayes_learn.txt"
            ...
            resFileName =  args.model + '_' +  args.dataset + "_novelty_bayes_test.txt"
            line = "ModelDir,Model,FewShotClassifier,Novelty,Way,Shot,Query,Accuracy,Precision,Recall,F1,TP,FP,FN,Method,Threshold,Percentage,Alpha,ModelName\n"
            if os.path.exists(resDir+subDir+resFileName):
                resFile = open(resDir+subDir+resFileName, "a")
            else:
                resFile = open(resDir+subDir+resFileName, "w")
          ...
          #      for use_novelty in [True, False]: # Test with and without novelty
            learned_threshold = novelty_th
            for n_shot in [5]: # Test with 5  shot               
                for use_novelty in [True]: # Test with novelty
                    ...
                    for percentage in [0.970, 0.975, 0.980, 0.985, 0.990, 0.995, 1.000, 1.005, 1.010, 1.015, 1.020, 1.025, 1.030]:
                    
                        novelty_th = percentage * learned_threshold
                        print("Threshold", novelty_th, percentage)
                        ...
                        for few_shot in few_shot_classifiers:
                            ...
                            line += args.threshold + ',' + str(threshold) + ',' + str(percentage*100) + ',' + str(args.alpha) + ',' + args.modelDir + '/' + modelName +  '\n'
                            print(line)
                            resFile.write(line)    
                            resFile.flush()
